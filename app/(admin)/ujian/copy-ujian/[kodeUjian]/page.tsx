...
import { BuatUjianAction } from "../../buat-ujian/BuatUjianAction";
import { useRef, useState } from "react";
import { redirect } from "next/navigation";
import { copyUjian } from "./CopyUjianAction";
import { useParams } from "next/navigation";

const BuatUjian = () => {
  const params = useParams();
  const kodeUjianParam = params.kodeUjian as string;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result?.message === "Success") {
      // Parameter kodeUjian yang pertama adalah kodeUjian yang dicopu, kodeUjian kedua adalah hasil dari func buat ujian (ujian baru)
      const copySoal = await copyUjian(
        kodeUjianParam as string,
        result.kodeUjian as string,
      );
      if (copySoal?.success) {
        redirect(`/dashboard`);
      } else {
        alert(copySoal?.message);
        // alert("Error di response copySoal");
      }
